...
export abstract class BaseAmmDexAnalyzer {
  ...
  protected abstract swapOrders(
    transaction: Transaction
  ): Promise<LiquidityPoolSwap[]> | LiquidityPoolSwap[];

  protected abstract depositOrders(
    transaction: Transaction
  ): Promise<LiquidityPoolDeposit[]> | LiquidityPoolDeposit[];

  protected abstract withdrawOrders(
    transaction: Transaction
  ): Promise<LiquidityPoolWithdraw[]> | LiquidityPoolWithdraw[];

  protected zapOrders?(
    transaction: Transaction
  ): Promise<LiquidityPoolZap[]> | LiquidityPoolZap[];

  /**
   * Attempts to retrieve a liquidity pool from its identifier.
   */
  protected async liquidityPoolFromIdentifier(
    identifier: string
  ): Promise<LiquidityPool | undefined> {
    const cacheInstance: LiquidityPool | undefined =
      await this.app.cache.getKey(identifier);
  ...
  protected spentOperationInputs(transaction: Transaction): OperationStatus[] {
    const indexes: number[] = transaction.redeemers
    ...
    return transaction.inputs.reduce(
      (spentInputs: OperationStatus[], input: Utxo, index: number) => {
   ...
   */
  protected cancelledOperationInputs(
    transaction: Transaction,
    orderAddresses: string[],
    redeemerDatum: string,
    referenceHashes: string[] = []
  ): OperationStatus[] {
    const containsOrderAddress: boolean =
      transaction.scriptHashes?.some((scriptHash: string) => {
        return (
          orderAddresses.includes(
            credentialToAddress('Mainnet', scriptHashToCredential(scriptHash))
          ) || orderAddresses.includes(scriptHash)
        );
      }) ?? false;
    const containsReference: boolean =
    ...
    return transaction.inputs.reduce(
      (cancelInputs: OperationStatus[], input: Utxo, index: number) => {
        const redeemer: Redeemer | undefined = transaction.redeemers.find(
          (redeemer: Redeemer) => {
            return (
              redeemer.validator.index === index &&
              redeemer.redeemer === redeemerDatum
            );
          }
        );

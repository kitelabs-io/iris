...
export class AxoAnalyzer extends BaseOrderBookDexAnalyzer {
  ...
  protected orders(
    ...
    const orderTokenBalance: AssetBalance | undefined =
      transaction.outputs[0].assetBalances.find(
        (balance: AssetBalance) =>
          balance.asset.policyId === tokenPolicyId && balance.quantity === 1n
      );
    const tokenSentToUser: boolean =
      transaction.outputs.length > 1 &&
      transaction.outputs[1].assetBalances.some(
        (balance: AssetBalance) =>
          balance.asset.policyId === tokenPolicyId && balance.quantity === 1n
      );

 ...
 */
const ORDER_CONTRACT_ADDRESS: string =
  'addr1wxaptpmxcxawvr3pzlhgnpmzz3ql43n2tc8mn3av5kx0yzs09tqh8';
const POOL_CONTRACT_ADDRESS: string =
      ...
      transaction.outputs.map((output: Utxo) => {
        return new Promise(async (resolve, reject) => {
          ...
          try {
            ...
            if (!existingPool)
              return reject(
                `Unable to find ${Dex.SundaeSwap} pool with identifier ${datumParameters.PoolIdentifier}`
              );
            ...
            if (output.assetBalances.length > 0) {
              ...
              swapInAmount =
                ...
                BigInt(datumParameters.ScooperFee as number) -
                DEPOSIT_FEE;
      ...
      .map((output: Utxo) => {
        ...
        try {
          ...
          return LiquidityPoolState.make(
            ...
            (Number(datumParameters.LpFeeNumerator) /
              Number(datumParameters.LpFeeDenominator)) *
              100,
            transaction.blockSlot,
            transaction.hash,
